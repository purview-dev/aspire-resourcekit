...
2. Skips the entire release if `v<version>` already exists (so re-merging to `main`, or merging `main`
...
5. Creates the `v<version>` GitHub release with generated release notes and attaches the package
   artifacts.
